...
class MpesertaController extends Controller {

	protected $layout = "layouts.main";
	protected $data = array();
	public $module = 'mpeserta';
	static $per_page	= '10';

	public function __construct()
	{
		parent::__construct();
		$this->model = new Mpeserta();
			...
			default:
				...
				if ($validator->passes())
				{
					...
					return redirect( $this->module .'?'. $this->returnUrl() )->with('message',__('core.note_success'))->with('status','success');
						...
						//dd($this->module.'/'. $request->input(  $this->info['key'] ).'/edit');
	...
	public static function display(  )
	{
		...
		if($mode == 'view')
		{
			...
			if($row)
			{
				...
				$data['id'] = $id;
				return view('mpeserta.public.view',$data);
			}
		}
		else {
			...
			return view('mpeserta.public.index',$data);
		}
	...
	function store_public( $request)
	{
		...
		if ($validator->passes()) {
			$data = $this->validatePost(  $request );
			$data['id_sso'] = \Auth::user()->sso_uuid;
